...
{
    [TestFixture]
    public class PackSetTests
    {
        [Test]
        public void ConstructorTest()
        {
            var collection = new PackSet<int>(1, 2, 3, 4, 5);
        ...
        public void IsTest()
        {
            var collection1 = new PackSet<int>(1, 2, 3, 4, 5);
            var collection2 = new PackSet<int>(1, 2, 3, 4, 5);

            Assert.That(collection1.Is(collection2), Is.True);
            collection2 = new PackSet<int>(2, 2, 3, 4, 5);
            Assert.That(collection1.Is(collection2), Is.False);
            collection1 = new PackSet<int>(2, 2, 3, 4, 5);
            Assert.That(collection1.Is(collection2), Is.True);
        }
        ...
        {
            var collection = new PackSet<int>(1, 2, 3, 4, 5);
            var bytes = collection.ToPackBytes(false);

            Assert.That(bytes, Is.Not.Null);

            Console.WriteLine(bytes.Length);

            var collection1 = bytes.FromPackBytes<PackSet<int>>(false);
